...
(serial archs only). LayerNorm is blind AGC --- hold it still and ablation damage grows 2--8$\times$; the
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f0_datasheet.png}\end{center}
\texttt{p1\_f0\_datasheet} --- reused across papers via the figure map in \texttt{ecc\_repr/figs\_dist.py} (L44); regenerate with
\texttt{paper1/build.sh} or \texttt{scripts/build.sh figs}. Figure CSVs land in \texttt{artifacts/figdata/}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f1_interference.png}\end{center}
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f6_despreading.png}\end{center}
\texttt{p1\_f1\_interference} --- reused via the figure map in \texttt{figs\_dist.py} (L44); \texttt{p1\_f6\_despreading} ---
\texttt{ecc\_repr/figs\_papers.py::p1\_f6\_despreading} (L208). Regenerate: \texttt{paper1/build.sh}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f2_l2.png}\end{center}
\texttt{p1\_f2\_l2} --- reused via the figure map in \texttt{figs\_dist.py} (L45). Regenerate: \texttt{paper1/build.sh}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f4_diversity.png}\end{center}
Fading panels inside \texttt{ecc\_repr/figs\_papers.py::p1\_f4\_diversity} (L226). Regenerate:
\texttt{paper1/build.sh}, or directly \texttt{.venv/bin/python -m ecc\_repr.figs\_papers p1\_f4}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f7_agc.png}\end{center}
Knockout/diversity panels: \texttt{figs\_papers.py::p1\_f4\_diversity} (L226); the AGC decomposition and gain
law: \texttt{figs\_papers.py::p1\_f7\_agc}. Regenerate: \texttt{paper1/build.sh}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f5_receivers.png}\end{center}
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p1_f8_crb.png}\end{center}
\texttt{figs\_papers.py::p1\_f5\_receivers} (L291) and \texttt{figs\_papers.py::p1\_f8\_crb}; regenerate
\texttt{paper1/build.sh} or \texttt{.venv/bin/python -m ecc\_repr.figs\_papers p1\_f5 p1\_f8}.
...
\emph{magnitude} does, the companion paper's mechanism (\texttt{paper2/SURVEY.md}). Model-specific, restated as such: "MLPs lead at every depth," "the
...
\item \textbf{"LayerNorm destroys absolute amplitude"} (a working assumption of this program, corrected by its own follow-up measurement): pre-LN per-token scale is recoverable from the post-LN stream at R$^{2}$ 0.91--0.99 --- LN re-encodes what it divides out. (\texttt{pilot\_exists.json}, measured in this repository's follow-up program --- \texttt{RESULTS\_level\_plan.md}.)
\end{itemize}
\section{References}
...
\item \textbf{This file is the paper.} \texttt{main.tex} and \texttt{main.pdf} are generated from it by \texttt{scripts/md2tex.py} (run by \texttt{paper1/build.sh}); the double-column LaTeX layout is retired. Both the markdown and the generated tex are machine-checked against manifests (\texttt{survey\_manifest.json}, \texttt{paper1\_manifest.json}) by \texttt{tests/test\_paper\_manifests.py}: every number with a decimal point must appear in a committed result JSON assigned to its section, or carry an allowlisted reason.
