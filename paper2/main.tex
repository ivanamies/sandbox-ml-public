...
\textbf{TL;DR.} Steering fails like a satellite amplifier: past a measurable knee the vector arrives at
...
0.83$\to$0.93 at 64$\times$. Cacheable, because the channel is \textasciitilde{}96\% memoryless.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p2_f1_mechanism.png}\end{center}
\texttt{ecc\_repr/figs\_papers.py::p2\_f1\_mechanism} (L47); regenerate \texttt{paper2/build.sh}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p2_f2_fix.png}\end{center}
\texttt{figs\_papers.py::p2\_f2\_fix} (L82); regenerate \texttt{paper2/build.sh}.
...
registered follow-up work in \texttt{paper2/NOTES.md}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p2_f3_memory_gate.png}\end{center}
\texttt{figs\_papers.py::p2\_f3\_memory} (L111); regenerate \texttt{paper2/build.sh}.
...
\textbf{Images.} 
\begin{center}\includegraphics[width=\linewidth]{survey_figs/p2_f4_amfm.png}\end{center}
\texttt{figs\_papers.py::p2\_f4\_amfm} (L135); regenerate \texttt{paper2/build.sh}.
...
(\texttt{paper1/SURVEY.md}). Supplies the wire datasheet and the two-tone linearity budget of §2.
...
\begin{itemize}\setlength\itemsep{1pt}
\item \textbf{Provenance.} Experiments, code, and this document by Claude (Fable 5), directed by ivanamies, who set the framing, the failure criteria ("kill bars"), and the editorial policy. Predictions were registered --- committed to git in \texttt{plans\_*.md} and \texttt{paper2/NOTES.md} --- before results.
\item \textbf{This file is the paper.} \texttt{main.tex} and \texttt{main.pdf} are generated from it by \texttt{scripts/md2tex.py} (run by \texttt{paper2/build.sh}). Both are machine-checked against manifests (\texttt{survey\_manifest.json}, \texttt{paper2\_manifest.json}) by \texttt{tests/test\_paper\_manifests.py}.
